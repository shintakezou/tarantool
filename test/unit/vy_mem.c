...
static const char *
msgpack(struct region *gc, size_t *p_size, const char *format, ...)
{
	va_list ap;
	va_start(ap, format);
	size_t required = mp_vformat(NULL, 0, format, ap);
	va_end(ap);
	char *buf = region_alloc(gc, required);
	assert(buf != NULL);
	va_start(ap, format);
	size_t written = mp_vformat(buf, required, format, ap);
	va_end(ap);
...
{
	size_t size;
	const char *data = msgpack(&fiber()->gc, &size, "[%d]", key);
	struct tuple *stmt = vy_stmt_new_replace(mem->format,
						 data, data + size);
	assert(stmt != NULL);
